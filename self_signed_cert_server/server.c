...
int main(void)
{
    ...
    SSL_CTX_load_verify_locations(sslctx, "cert/ca.crt", NULL);
    int use_cert = SSL_CTX_use_certificate_file(sslctx, "cert/servercert.pem", SSL_FILETYPE_PEM);
    if (1 != use_cert) {
        fprintf(stderr, "Error: cert import failed\n");
        return -4;
    }
    int use_priv = SSL_CTX_use_PrivateKey_file(sslctx, "cert/serverkey.pem", SSL_FILETYPE_PEM);
    if (1 != use_priv) {
